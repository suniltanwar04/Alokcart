...
class HomeController extends Controller
{
    ...
    {
        $lead_check = DB::table('leads')->where('leadEmail',$request->email)->orwhere('leadContact',$request->phone)->first();    
        if($lead_check) {
        ...
        $lead->leadContact=$request->phone;
        $lead->leadProduct=$request->product_id;
        $lead->leadProductTitle=$request->product;
        $lead->leadProductQty=$request->quantity;
        $lead->leadStatus=1;
