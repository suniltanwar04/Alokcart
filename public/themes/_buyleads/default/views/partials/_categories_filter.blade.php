        ...
        @if(Request::is('buy-leads'))
            <li>
                @if(Request::has('ingrp'))
                    ...
                    <h4>{{ $category->name }}</h4>
                    <?php
                        $t_categories = $products_new->pluck('product.categories')->flatten()->unique();
                        ...
                        @foreach($category->categories as $category)
                            @if(in_array($category->slug, $t_categories))
                                <li>
                                    ...
                                    <div class="checkbox">
                                        <label>
                                            <!-- <input name="in[{{$category->slug}}]" class="i-check filter_opt_checkbox" type="checkbox" {{ Request::has('in.$category->slug') ? 'checked' : '' }}> {{ $category->name }} -->
