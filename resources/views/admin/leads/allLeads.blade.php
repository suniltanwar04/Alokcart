...
@section('content')
	<div class="box">
	    <div class="box-header with-border">
		  ...
		  <span class="pull-right">
				<a href="javascript:void(0)" data-link="{{ route('admin.leads.lead.create') }}" class="ajax-modal-btn btn btn-info btn-flat">+Add Lead</a>
		  </span>
		  @endif
		  <span class="pull-right">
				<a class="btn btn-md btn-info" href="{{url('/admin/leads')}}"><i class = "fa fa-trash"></i> Reset Filter</a>
	  	</span>
		  <style>
					#leadDateFilter li{
						margin: 0px 5px;
			...
			<span class="pull-right filterContainer" id="leadDateFilter">
				{{ Form::open(['url'=>'/admin/leads/','method' => 'get' ])}}
	      ...
	      <table class="table table-hover table-2nd-no-sort datatable" id="sortable" >
	        <thead>
		        <tr>
					...
					<th  width="25%">{{ trans('app.product') }}</th>
					<th>{{ trans('qty') }}</th>
					{{-- @if(Auth::user()->role_id!=3)
		        ...
		        @foreach($Leads as $leads )
					<tr id="{{ $leads->leadId }}">
						...
						<td class="row-options">
							...
							{!! Form::close() !!}
							@endif
							<a href="javascript:void(0)" data-link="{{ route('admin.leads.lead.report', $leads->leadId) }}"  class="ajax-modal-btn"><i data-toggle="tooltip" data-placement="top" title="Report" class="fa fa-address-card"></i></a>&nbsp;
					</td>
			        </tr>
		        @endforeach
	        </tbody>
